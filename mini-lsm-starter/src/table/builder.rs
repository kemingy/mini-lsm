...
use crate::{block::BlockBuilder, key::Key, key::KeySlice, lsm_storage::BlockCache};
...
impl SsTableBuilder {
    ...
    pub fn add(&mut self, key: KeySlice, value: &[u8]) {
        ...
        self.renew_block_builer();
        self.first_key.extend_from_slice(key.raw_ref());
        assert!(
            self.builder.add(key, value),
            "first key should be added to builder successfully"
        );
        self.last_key.clear();
        self.last_key.extend_from_slice(key.raw_ref());
    }

    fn renew_block_builer(&mut self) {
